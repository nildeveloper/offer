...
//
//    
//    // Node  存放元素的key,value,hash值,next下一个链表节点的引用。用于bucket为链表时。
...
//    /* ---------------- TreeNodes -------------- */
//
//   // Node是链表中的元素，而TreeBin和TreeNode也继承自Node节点，也自然继承了next属性，
//   // 同样拥有链表的性质，其实真正在存储时，红黑树仍然是以链表形式存储的，只是逻辑上
//   // TreeBin和TreeNode多了支持红黑树的root，first, parent，left，right，red属性，
//   // 在附加的属性上进行逻辑上的引用和关联，也就构造成了一颗树。
...
//
//        // 红黑树遍历
//        // 就是二叉树查找，红黑树是按hash值的大小来构造左子节点和右子节点的，
